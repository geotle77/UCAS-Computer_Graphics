...
float Sphere::Intersect( const Ray &ray, glm::vec3 *hitNorm, glm::vec3 *hitPos){
      // 将光线的表达式p=e+t·d带入球的方程(p-c)^2-R^2=0中进行求解
    glm::vec3 dist = ray.ori - center;
    ...
    if(hitNorm != NULL)
        *hitNorm = (*hitPos - center) / radius;
    return t;
}

float Cube::Intersect(const Ray &ray, glm::vec3 *hitNorm, glm::vec3 *hitPos) {
    glm::vec3 rayDirInv = 1.0f / ray.dir; // 光线方向的倒数

    // 计算光线进入和离开立方体的参数
    glm::vec3 tMin = (center - 0.5f * edgeLength - ray.ori) * rayDirInv;
    glm::vec3 tMax = (center + 0.5f * edgeLength - ray.ori) * rayDirInv;
    ...
    // 如果 tEnter 大于 tExit，说明光线未与立方体相交
    ...
    // 计算交点坐标
    if (hitPos != NULL) {
        *hitPos = ray.ori + tEnter * ray.dir;
    }

    // 计算法向量
    if (hitNorm != NULL) {
        // 判断哪个面相交，并计算法向量
        // 这里只是简化的例子，实际情况可能需要更多的逻辑判断
        glm::vec3 hitPoint = *hitPos - center;

        if (glm::abs(hitPoint.x) > glm::abs(hitPoint.y) && glm::abs(hitPoint.x) > glm::abs(hitPoint.z)) {
            *hitNorm = glm::vec3(glm::sign(hitPoint.x), 0.0f, 0.0f);
        } else if (glm::abs(hitPoint.y) > glm::abs(hitPoint.x) && glm::abs(hitPoint.y) > glm::abs(hitPoint.z)) {
            *hitNorm = glm::vec3(0.0f, glm::sign(hitPoint.y), 0.0f);
        } else {
            *hitNorm = glm::vec3(0.0f, 0.0f, glm::sign(hitPoint.z));
        }
    }
...
bool Scene::SceneIntersect(const Ray &ray,
    ...
    for (int i = 0; i < objects.size(); ++i) {
        glm::vec3 _hit_pos;
        glm::vec3 _hit_norm;
        float t = objects[i]->Intersect(ray, &_hit_norm, &_hit_pos);
...
bool Scene::ShadowRayIntersect(const Ray &shadow_ray){
    ...
    for (int i = 0; i < objects.size(); ++i) {
        float t = objects[i]-> Intersect(shadow_ray, NULL, NULL);
...
void renderScene(Scene &mainscene,float viewWidth, float viewHeight, void(*handleFunc) (float, float, glm::vec3)) {
    ...
    for (int viewx = 0; viewx < fov; viewx++) {
        for (int viewy = 0; viewy < fov; viewy++) {
            Ray viewRay = mainscene.camera.getRay(viewx, viewy, fov);
            glm::vec3 pixelColor = mainscene.Trace(viewRay);

...
### 注意
可以看到在渲染模型的时候，我并不是直接用openmesh计算得到的法向量，而是重新计算了面和顶点的法向量，这并不是多此一举。相反正是因为在openmesh中，各
个面的法线不一定是同向的，也就是说这些面的方向不太一致，**可能朝外也可能朝内。**

当然上述只是我的猜测，可能有其他的解决办法，但总之如果直接使用对应顶点的法线就会是下图的渲染效果：
![bad rendering](./example/rendering_BN.png)
